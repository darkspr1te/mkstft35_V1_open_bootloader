...
{
    if (HAL_GPIO_ReadPin(XPT2046_IRQ_GPIO_Port, XPT2046_IRQ_Pin)==0)
        {
            HAL_Delay(10);
        }
...
bool XPT2046_TouchGetCoordinates(uint16_t* x, uint16_t* y)
{
    ...
    for(uint8_t i = 0; i < 16; i++)
    {
        ...
        if (HAL_SPI_Transmit(&XPT2046_SPI_PORT, (uint8_t*)cmd_read_y, sizeof(cmd_read_y), HAL_MAX_DELAY) != HAL_OK)
...
{ 
 ...
 { 
     uint16_t tx,ty;
     XPT2046_TouchGetCoordinates(&tx,&ty);
     return tx;
 }
